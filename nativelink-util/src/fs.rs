...
pub async fn hard_link(src: impl AsRef<Path>, dst: impl AsRef<Path>) -> Result<(), Error> {
    ...
    call_with_permit(move |_| -> Result<(), Error> {
        ...
        // file. Falls back to hard_link on cross-volume / unsupported errors.
        #[cfg(target_os = "macos")]
        {
            ...
            if let (Ok(src_c), Ok(dst_c)) = (
                ...
                // Fall through to hard_link only on errors that indicate the
                // filesystem doesn't support clonefile (cross-volume, non-APFS).
